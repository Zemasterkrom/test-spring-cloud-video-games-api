...
API_URL = $(echo "${API_URL}" | sed -E "s/(http[s]?://)(.*)/\1-get\2/g")
if ! [ $(echo "${API_URL}" | grep -E "http[s]?://.*") ]; then
  API_URL = "http-get://${API_URL}"
fi

wait-on ${API_URL:-http-get://localhost:8080}/video-games/all -t 120000 -i 10000 && ng serve --port ${FRONT_SERVER_PORT:-4200}